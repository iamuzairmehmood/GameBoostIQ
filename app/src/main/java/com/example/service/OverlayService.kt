...
class OverlayService : Service() {
    ...
    companion object {
        const val ACTION_START = "com.example.gameboost.OVERLAY_START"
        const val ACTION_STOP = "com.example.gameboost.OVERLAY_STOP"
    }
    ...
    private fun showOverlay() {
        ...
        tvRam = createMetricRow(linear, "RAM", "3.2 GB", "#F0F4F8")
    ...
    private fun startStatsUpdates() {
        ...
        updateJob = scope.launch {
            ...
            while (isActive) {
                ...
                if (stats != null) {
                    val fpsStr = if (stats.fps != null) "${stats.fps}" else "60"
                    tvFps?.text = fpsStr
                    tvTemp?.text = "${"%.1f".format(stats.temperatureCelsius)}°C"
                    tvPing?.text = "${stats.pingMs} ms"
                    tvRam?.text = "${stats.ramUsedGb} GB"
                    tvRefresh?.text = "${stats.currentRefreshRateHz.toInt()} Hz"
                    tvBattery?.text = "${stats.batteryPercent}%"

                    compactText?.text = "⚡ $fpsStr FPS • ${stats.pingMs}ms • ${"%.0f".format(stats.temperatureCelsius)}°C"
                }
                delay(1000L)
            }
        }
